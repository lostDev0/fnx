...
namespace fnx
{
	...
	public:
		...
		size_t num_available() { return _available.size(); }

		/// @brief Return the nubmer of used handles.
		size_t num_used() const { return max_handles - _available.size(); }

		/// @brief Return all available ids.
		auto& available() { return _available; }
		...
		const auto& in_use() const { return _used; }
	private:
