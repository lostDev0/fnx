...
public:
    ...
    fnx::vector2 calculate_texture_model_info( fnx::alignment align, const fnx::vector2& size_limits,
            float font_size_in_pixels, const std::string& text, std::vector<float>& model_coords,
            std::vector<float>& text_coords, float window_width, float window_height, fnx::vector2& cursor,
            std::vector<std::pair<float, std::string>>& lines );
    ...
    auto get_char_info( const char c )
    {
        return _character_info[c];
    }
...
protected:
    ...
    fnx::vector2 calculate_string_size( const fnx::vector2& size_limits, const std::string& text,
                                        float font_height_in_pixels,
                                        float window_width, float window_height, std::vector<std::pair<float, std::string>>& lines );

    void init( fnx::asset_manager<fnx::texture>& textures, const std::string& texture_file_path,
