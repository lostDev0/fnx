...
namespace fnx
{
	...
	public:
		...
		auto count() { return _layers.size(); }
		...
		auto get_layer_at_position_event(size_t index) { return _layers[index]; }

		/// @brief Returns every layer.
		const auto& get_layers() const { return _layers; }

		/// @brief Returns the opengl entity layer.
		auto get_entity_layer() { return _layers[0u]; }
