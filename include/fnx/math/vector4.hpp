...
public:
    ...
    inline auto xy() const
    {
        return fnx::vector2{x, y};
    }
    inline auto zw() const
    {
        return fnx::vector2{z, w};
    }
    inline auto r() const
    {
        return x;
...
// Constructor with arguments
RP3D_FORCE_INLINE vector4::vector4( decimal newX, decimal newY, decimal newZ, decimal newW ) : x( newX ), y( newY ),
    z( newZ ), w( newW )
{
...
// Set all the values of the vector
RP3D_FORCE_INLINE void vector4::setAllValues( reactphysics3d::decimal newX, reactphysics3d::decimal newY,
        reactphysics3d::decimal newZ, reactphysics3d::decimal newW )
{
    x = newX;
    y = newY;
    z = newZ;
...
{
    return "vector4(" + std::to_string( x ) + "," + std::to_string( y ) + "," + std::to_string( z ) + "," + std::to_string(
               w ) + ")";
}
...
RP3D_FORCE_INLINE bool approxEqual( const vector4& vec1, const vector4& vec2,
                                    reactphysics3d::decimal epsilon = MACHINE_EPSILON )
{
