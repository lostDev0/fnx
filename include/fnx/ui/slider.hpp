...
public:
    ...
    slider( fnx::widget_handle_t<fnx::progress_bar> foreground, fnx::widget_handle_t<fnx::block> anchor,
            const std::string& name = create_type_name<slider>() )
        : widget( name )
    ...
    auto get_progress()
    {
        return _foreground ? _foreground->get_progress() : 0.f;
    }

    auto get_fill_direction()
    {
        return _foreground ? _foreground->get_fill_direction() : fill_direction::left_to_right;
    }
