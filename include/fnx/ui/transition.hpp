...
public:
    ...
    auto is_done() -> bool const
    {
        return _rewind ? _current_time <= 0.0 : _current_time >= _duration;
    }

    /// @brief Is the transition paused.
    auto is_paused() -> bool const
    {
        return _paused;
    }

    /// @brief Is the transition backwards incrementing.
    auto is_backward() -> bool const
    {
        return _rewind;
    }
...
public:
    ...
    void set_delay( double delay_in_seconds )
    {
        _delay = delay_in_seconds;
    }
    auto get_delay() const
    {
        return _delay;
    }
    auto get_duration() const
