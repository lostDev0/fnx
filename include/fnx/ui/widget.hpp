...
namespace fnx
{
    ...
    public:
        ...
        auto get_x() const { return _current_bounds.x; }

        /// @brief Get the widget local y coordinate in world space (opengl).
        auto get_y() const { return _current_bounds.y; }

        /// @brief Get the widget width in world space (opengl).
        auto get_width() const { return _current_bounds.z; }

        /// @brief Get the widget height coordinate in world space (opengl).
        auto get_height() const { return _current_bounds.w;  }
        ...
        auto get_abs_x() -> reactphysics3d::decimal { return nullptr == _parent ? get_x() : (_parent->get_abs_x() + get_x()); }

        /// @brief Get the widget global y coordinate in world space (opengl).
        auto get_abs_y() -> reactphysics3d::decimal { return nullptr == _parent ? get_y() : (_parent->get_abs_y() + get_y()); }
        ...
        void set_strict_size(reactphysics3d::decimal width, reactphysics3d::decimal height) { _strict_size = fnx::vector2{ width, height }; }
        ...
        const auto& get_strict_size() { return _strict_size; }

        /// @brief Set the widget's alpha transparency.
        /// @note Primarily used for transitions.
        void set_alpha(reactphysics3d::decimal alpha) { _current_alpha = alpha; }

        /// @brief Get this widget's current alpha.
        auto get_alpha() { return _current_alpha; }

        /// @brief Gets the position without animator manipulation
        auto get_original_position() const { return _original_bounds.xy(); }

        /// @brief Gets the size without animator manipulation
        auto get_original_size() const { return _original_bounds.zw(); }
        
        /// @brief Gets the position with animator manipulation (slide)
        auto get_current_position() const { return _current_bounds.xy(); }
        
        /// @brief Gets the size with animator manipulation (scale)
        auto get_current_size() const { return _current_bounds.zw(); }

        /// @brief Gets the alpha value with animator manipulation (fade)
        auto get_original_alpha() const { return _original_alpha; }
        ...
        void set_parent(widget& parent) { _parent = &parent; }

        /// @brief Returns whether this widget has a parent.
        auto has_parent() { return nullptr != _parent; }

        auto get_parent() { return _parent; }

        /// @brief Set size and position constraints.
        void set_constraints(const constraints& constraints) { _constraints = constraints; }
        ...
        auto& get_animator() { return _animator; }

        /// @brief Notify the widget that it's parent dimensions changed.
        virtual void on_parent_change(reactphysics3d::decimal parent_x, reactphysics3d::decimal parent_y, reactphysics3d::decimal parent_width, reactphysics3d::decimal parent_height);
        ...
        const bool& is_checked() const { return _checked; }
        void set_checked(const bool& val) { _checked = val; }

        /// @brief Can the widget be checked.
        const bool& is_checkable() const { return _checkable; }

        void set_checkable(const bool& checkable) { _checkable = checkable; }
        ...
        void toggle() { if (is_checkable()) { _checked ? uncheck() : check(); } }

        /// @brief Does the widget have children.
        bool has_children() const { return !_children.empty(); }

        /// @brief Get the children of the widget.
        const std::vector<fnx::widget_handle>& get_children() const { return _children; }
        
        /// @brief Get the number of children.
        auto num_children() const { return _children.size(); }
        ...
        auto get_state() { return _state; }

        /// @brief Is the cursor currently over the widget
        auto is_cursor_over() { return _mouse_over; }

        /// @brief Is the widget currently undergoing manipulation.
        auto is_active() -> bool { return _active; }
        ...
        auto is_pressed() { return _state == state::press; }
        ...
        virtual bool do_mouse_move(double x, double y) { return false; }
        virtual bool do_mouse_enter() { return false; };
        virtual bool do_mouse_exit() { return false; };
        virtual bool do_mouse_press(FNX_BUTTON button, double x, double y) { return false; }
        virtual bool do_mouse_release(FNX_BUTTON button, double x, double y) { return false; }
        virtual bool do_key_press(FNX_KEY key) { return false; }
        virtual bool do_key_release(FNX_KEY key) { return false; }
        virtual bool do_key_repeat(FNX_KEY key) { return false; }
        virtual void do_active() {}
        virtual void do_inactive() {}
        virtual bool do_scroll(double x, double y) { return false; }
    ...
    protected:		
        ...
        virtual void do_made_invisible() { _animator_hidden = true; }
    };
