...
void font::init( fnx::asset_manager<fnx::texture>& textures, const std::string& texture_file_path,
                 const std::string& char_map_file_path )
{
    // disable mipmapping
    // mipmapping can be enabled if text is drawn at various depths and may gain some quality improvements
    texture::config cfg;
    cfg._enable_mip = false;
    _texture = textures.get( texture_file_path,
                             cfg ); //, 4, Format::RGBA, Format::RGBA, Filter::Linear, Attachment::None, true )
    _line_height = { 0 };
    _base = { 0 };
...
fnx::vector2 font::calculate_string_size( const fnx::vector2& size_limits, const std::string& text,
        float font_height_in_pixels,
        float window_width, float window_height, std::vector<std::pair<float, std::string>>& lines )
{
    ...
    float width_of_small_char = static_cast<float>( dot_char.xadvance ) * width_to_opengl *
                                font_scale;	/// opengl width of the . character
    ...
    for ( auto idx = 0u; idx < text.size(); ++idx )
    {
        ...
        if ( save_line )
        {
            if ( current_line_whole_words_end > current_line_whole_words_start )
            {
                ...
                current_line_whole_words_start =
                    current_line_whole_words_end;	// advance the next string to the remaining character string
            }
        }
...
fnx::vector2 font::calculate_texture_model_info( fnx::alignment align, const fnx::vector2& size_limits,
        float font_height_in_pixels,
        const std::string& text, std::vector<float>& model_coords, std::vector<float>& text_coords,
        float window_width, float window_height, fnx::vector2& cursor_in_opengl_coords,
        std::vector<std::pair<float, std::string>>& line_map )
{
