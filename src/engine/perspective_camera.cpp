...
void perspective_camera::pitch(const fnx::angle& angle)
{
	...
	{
		/*
		reactphysics3d::Vector3 right = (_up.cross(_front)).get_normal();
		...
		_front = reactphysics3d::Vector3(rotation.x, rotation.y, rotation.z).get_normal();
...
void perspective_camera::rotate_y(const fnx::angle& angle)
{
	...
	{
		/*
		static const reactphysics3d::Vector3 up(0.0f, 1.0f, 0.0f);
		...
		_front = reactphysics3d::Vector3(front_rotation_vector.x, front_rotation_vector.y, front_rotation_vector.z).get_normal();
		_up = reactphysics3d::Vector3(up_rotation_vector.x, up_rotation_vector.y, up_rotation_vector.z).get_normal();
		update_view_matrix();
		*/
	}
}
...
void perspective_camera::rotate_z(const fnx::angle& angle)
{
	...
	{
		/*
		static const reactphysics3d::Vector3 up(0.0f, 0.0f, 1.0f);
		auto rotation = fnx::matrix_rotate(fnx::matrix4x4::identity(), angle, up);
		auto front_rotation_vector = fnx::matrix_to_vector(rotation * fnx::matrix<float, 4, 1>(fnx::vec4f(_front, 0.f)));
		auto up_rotation_vector = fnx::matrix_to_vector(rotation * fnx::matrix<float, 4, 1>(fnx::vec4f(_up, 0.f)));
		_front = reactphysics3d::Vector3(front_rotation_vector.x, front_rotation_vector.y, front_rotation_vector.z).get_normal();
