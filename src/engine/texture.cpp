...
namespace fnx
{
    ...
    texture::texture(unsigned int width, unsigned int height, unsigned int channels, fnx::format format,
        fnx::format internal_format, fnx::filter filter, fnx::attachment attachment, bool clamp, bool enable_mip_mapping, float mip_bias,
    ...
    void texture::init()
    {
        ...
        if (_enable_mip)
        {
            ...
            glTexImage2D(_impl->_texture_target, 0, _internal_format, info._width, info._height, 0, _format, GL_UNSIGNED_BYTE, info._data);
    ...
    void texture::pixel(unsigned int row, unsigned int col, char& r, char& g, char& b, char& a) const
    {
        ...
        if (info._size > 0 && info._height > 0 && info._width > 0 && row < info._height && col < info._height)
        {
            ...
            char* pixelOffset = &static_cast<char*>(info._data)[(min(row, info._width - 1) * info._width + col) * info._channels];
