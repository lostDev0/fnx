	...
	void slider::update(double seconds)
	{
		if (_anchor && _foreground)
		{
			...
			if (direction == fill_direction::left_to_right)
			{
				...
				anchor_constraint.add(fnx::relative_horz_constraint(progress - ((_anchor->get_width() / _foreground->get_width()) / 2.f)));
			}
			else if (direction == fill_direction::bottom_to_top)
			{
				anchor_constraint.add(fnx::center_horz_constraint());
				anchor_constraint.add(fnx::relative_vert_constraint(progress - ((_anchor->get_height() / _foreground->get_height()) / 2.f)));
			}
			else if (direction == fill_direction::right_to_left)
			{
				anchor_constraint.add(fnx::center_vert_constraint());
				anchor_constraint.add(fnx::relative_horz_constraint((1.f - progress) - ((_anchor->get_width() / _foreground->get_width()) / 2.f)));
			}
			else if (direction == fill_direction::top_to_bottom)
			{
				anchor_constraint.add(fnx::center_horz_constraint());
				anchor_constraint.add(fnx::relative_vert_constraint((1.f - progress) - ((_anchor->get_height() / _foreground->get_height()) / 2.f)));
			}
