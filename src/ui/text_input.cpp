namespace fnx
{
	...
	bool text_input::do_mouse_release(FNX_BUTTON btn, double x, double y)
	{
		...
		if (_mouse_over && is_active())
		{
			...
			auto num_lines = static_cast<unsigned int>(dy / _label_font->calculate_line_height(height_in_pixels, static_cast<float>(window_height)));
	...
	void text_input::process_key(FNX_KEY key)
	{
		...
		else if (key == FNX_KEY::FK_C && (window::is_key_pressed(FNX_KEY::FK_LEFT_CONTROL) || window::is_key_pressed(FNX_KEY::FK_RIGHT_CONTROL)))
		{
			// TODO : Copy current highlight
		}
		else if (key == FNX_KEY::FK_V && (window::is_key_pressed(FNX_KEY::FK_LEFT_CONTROL) || window::is_key_pressed(FNX_KEY::FK_RIGHT_CONTROL)))
		{
			// TODO : Past clipboard at current location
		}
		else if (key == FNX_KEY::FK_A && (window::is_key_pressed(FNX_KEY::FK_LEFT_CONTROL) || window::is_key_pressed(FNX_KEY::FK_RIGHT_CONTROL)))
		{
			// TODO : Select All
		}
		else
